...
interface MouseState {
  ...
  sx: number; sy: number; v: number; vs: number
...
export default function WavesCanvas() {
  ...
  useEffect(() => {
    ...
    const cfg: WavesConfig = {
      lineColor: 'rgba(244,241,232,.18)',
      waveSpeedX: 0.018,
      waveSpeedY: 0.008,
      waveAmpX: 34,
      waveAmpY: 18,
    ...
    let lines: WavePoint[][] = []
    const mouse: MouseState = { x: -10, y: 0, lx: 0, ly: 0, sx: 0, sy: 0, v: 0, vs: 0, a: 0, set: false }
    let raf = 0

    function setSize() {
      bounding = container.getBoundingClientRect()
      const dpr = Math.min(window.devicePixelRatio || 1, window.innerWidth <= 760 ? 1.15 : 1.5)
      canvas.width = Math.max(1, Math.floor(bounding.width * dpr))
      canvas.height = Math.max(1, Math.floor(bounding.height * dpr))
    ...
    function moved(p: WavePoint, withCursor = true) {
    ...
    function updateMouse(x: number, y: number) {
      const b = bounding
      mouse.x = x - b.left; mouse.y = y - b.top
      if (!mouse.set) {
  ...
  return (
    <div className="waves-root" aria-hidden="true">
      <div ref={containerRef} className="waves-inner">
