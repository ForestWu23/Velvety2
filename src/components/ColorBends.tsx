/**
 * ColorBends — ReactBits WebGL background, faithfully ported to TypeScript.
 * Source: https://reactbits.dev/backgrounds/color-bends
 * Three.js GLSL shader that renders flowing colour bands at a given rotation.
 */
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    // @ts-expect-error — Three.js runtime property
    ...
    // Listen on window so overlay siblings don't swallow events
  ...
  useEffect(() => {
    ...
    ;(mat.uniforms.uColors.value as THREE.Vector3[]).forEach((vec, i) => {
      if (i < arr.length) vec.copy(arr[i])
      else vec.set(0, 0, 0)
    })
    mat.uniforms.uColorCount.value = arr.length
    mat.uniforms.uTransparent.value = transparent ? 1 : 0
    rend?.setClearColor(0x000000, transparent ? 0 : 1)
